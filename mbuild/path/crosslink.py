    ...
    for ref_node, ref_coord in zip(ref_nodes, ref_coords):
        ...
        for idx in closest_paired_nodes:
            ...
            if len(selected_nodes) >= n_backbone_connections:
                ...
                if crosslinker.n_sites == 1 and len(selected_nodes) == 2:
                    selected_set_temp = set(selected_nodes)
                    check_mask_temp = np.ones(len(path.coordinates), dtype=bool)
                    for _idx in selected_set_temp:
                        check_mask_temp[_idx] = False
                    check_coords_temp = np.ascontiguousarray(
                        path.coordinates[check_mask_temp], dtype=np.float32
                    )
                    bb_coords = np.array(
                        [path.coordinates[n] for n in selected_nodes], dtype=np.float32
                    )
                    valid_overlaps, clink_centroid = _find_single_site_position(
                        bb_coords,
                        crosslink_bond_length,
                        check_coords_temp,
                        min_separation,
...
def _find_single_site_position(
    backbone_coords,
    bond_length,
    check_coords,
    min_separation,
    ...
    """Find the best overlap-free position for a single-site crosslinker.

    For two backbone beads A and B at distance d, positions exactly `bond_length`
    from both lie on a circle: midpoint M + h*(cos(t)*e1 + sin(t)*e2), where
    h = sqrt(bond_length^2 - (d/2)^2) and e1, e2 are perpendicular to AB.
    Sweeps this circle to find the position with maximum clearance from
    non-bonded beads.

    Parameters
    ----------
    backbone_coords : np.ndarray, shape (2, 3)
    bond_length : float
    check_coords : np.ndarray, shape (N, 3)
        All path coords excluding the two backbone beads.
    min_separation : float or None
    n_rotation_samples : int
    ...
    -------
    valid : bool
    best_position : np.ndarray, shape (3,)
    """
    pA = np.asarray(backbone_coords[0], dtype=np.float32)
    pB = np.asarray(backbone_coords[1], dtype=np.float32)
    ...
    M = (pA + pA + delta) * 0.5  # midpoint, PBC-unwrapped

    if d > 2.0 * bond_length:
        return False, M

    axis = (delta / d).astype(np.float32) if d > 1e-10 else np.array([1, 0, 0], dtype=np.float32)
    h = float(np.sqrt(max(0.0, bond_length ** 2 - (d * 0.5) ** 2)))

    perp1 = _get_perpendicular(axis)
    perp2 = np.cross(axis, perp1).astype(np.float32)
    perp2 /= np.linalg.norm(perp2)

    angles = np.linspace(0, 2 * np.pi, n_rotation_samples, endpoint=False)

    best_pos = M.astype(np.float32)
    best_min_dist = -np.inf
    found_valid = False

    for angle in angles:
        P = (M + h * (float(np.cos(angle)) * perp1 + float(np.sin(angle)) * perp2)).astype(np.float32)

        min_dist = _min_dist_pbc(P[np.newaxis], check_coords, pbc, box_lengths) if len(check_coords) > 0 else np.inf
