...
def fill_box(compound, n_compounds=None, box=None, density=None, overlap=0.2,
   ...
   been filled to the user's specifications to the best of PACKMOL's ability.
    ...
    # Uses tempfile package to create a temporary .xyz file
    filled_xyz = _new_xyz_file()

    # create a list to contain the file handles for the compound temp files
    compound_xyz_list = list() # empty list
    try:
        input_text = PACKMOL_HEADER.format(overlap, filled_xyz.name, seed)
            # overlap = tolerance in packmol.
            # filled_xyz.name = temporary xyz file of single compound
            # ^ is the output file generated by packmol
            # input_text is a variable containing only strings

        for comp, m_compounds, rotate in zip(compound, n_compounds, fix_orientation):
            # comp: compound from list (single or multiple)
            # m_compounds: number of compounds (list - single or multiple)
            # rotate: True/False (list - single or multiple)

            m_compounds = int(m_compounds)

            compound_xyz = _new_xyz_file() # temporary, blank .xyz file
            compound_xyz_list.append(compound_xyz) # newly created file added to list of temp files

<<<<<<< HEAD
            comp.save(compound_xyz.name, overwrite=True)
            PACKMOL_CONSTRAIN = packmol_constrain(rotate) 
=======
            comp.save(compound_xyz.name, overwrite=True) # Saves the mb Compound to the temp .xyz file

            # Appends to input_text string:
            # Passes in info about box dimensions, compound .xyz files, and number of compounds
            # Appends another entry for each kind of compound passed in.
>>>>>>> 48eadccf41bc59d4981099ebaa72a35fe4d63a4b
        ...
        _run_packmol(input_text, filled_xyz, temp_file)
            # Creates the .inp file from input_text, and runs packmol command
            # Saves the resulting packmol output based on filled_xyz, temp_file location
            # Create the topology and update the coordinates.
        filled = Compound() # New, empty mbuild compound
        filled = _create_topology(filled, compound, n_compounds)
            # popualtes new compound from compound and n_compound lists
        filled.update_coordinates(filled_xyz.name, update_port_locations=update_port_locations)
            # Uses the packmol created .xyz file to update positions in filled compound
        filled.periodicity = np.asarray(box.lengths, dtype=np.float32)
            # Provide information on the periodicity of the system (box lengths)
